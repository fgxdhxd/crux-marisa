...
static bool g_rx1619_restart_flag = false;
...
void rx1619_retry_id_auth(struct rx1619_chg *chip)
{
	rx1619_write(chip, PRIVATE_ID_CMD, REG_RX_SENT_CMD);
	rx1619_write(chip, AP_SENT_DATA_OK, REG_AP_RX_COMM);
}

void rx1619_send_device_auth(struct rx1619_chg *chip)
{
...
bool rx1619_is_vout_on(struct rx1619_chg *chip)
{
	...
	if((voltage > MIN_VOUT) && (voltage < MAX_VOUT))//4V~15V
	{
		vout_status = true;
	}
	else
...
static void rx1619_set_reverse_parameter(struct rx1619_chg *chip, u8 mode, u8 iin) {
	dev_info(chip->dev, "mode: %d, iin:%d\n", mode, iin);
	rx1619_write(chip, REVERSE_PARAM_CMD, REVERSE_PARAM_CMD_ADDR);
	rx1619_write(chip, mode, REVERSE_PARAM_MODE_ADDE);
	rx1619_write(chip, iin, REVERSE_PARAM_IIN_ADDR);
	return;
}
...
static void rx1619_set_reverse_fod(struct rx1619_chg *chip, u8 compensation, u8 threshold) {
...
static bool rx1619_update_fw_confirm_data(struct rx1619_chg *chip, u8 area, u8 confirm_data)
{
	u16 addr = 0;
	u8  addr_h, addr_l;
	u8 data[4] = {0, 0, 0, 0};
...
//	rx1619_write(chip, 0x19, 0x0018);
	...
	dev_err(chip->dev, "[rx1619] [%s] area=%d,addr=%d, g_fw_data_lenth=%d, count=%d\n", __func__ ,
				area, addr,g_fw_data_lenth,count);
...
CCC:
	...
	dev_err(chip->dev, "error_conut= %d, pass_count=%d, (sizeof(fw_data_rx)) = %ld \n",
				j, pass_count, (sizeof(fw_data_rx)));

	dev_info(chip->dev, "fw_data version=0x%x, 0x%x, 0x%x, 0x%x \n",
			0xff&(~fw_data[g_fw_data_lenth-4]), 0xff&(~fw_data[g_fw_data_lenth-3]),
			0xff&(~fw_data[g_fw_data_lenth-2]), 0xff&(~fw_data[g_fw_data_lenth-1]));
...
bool rx1619_download_area_firmware(struct rx1619_chg *chip, u8 area)
{
	...
	if (rx1619_check_firmware_version(chip, area) == true) {
		dev_err(chip->dev, "[rx1619] [%s] Same fw Version, no upgrade required! \n", __func__);
		return true;
	}
	msleep(20);
...
RETRY:
	...
	dev_err(chip->dev, "download_area:%d  success_cout=%d, g_fw_data_lenth=%d\n", area, ret, g_fw_data_lenth);
	if (g_fw_data_lenth == (ret*4)) {
		status = rx1619_update_fw_confirm_data(chip, area, 0x66);//0x66(check success)
		if (!status) {
			dev_err(chip->dev, "area:%d update firmware confirm data fail! \n", area);
			return false;
		} else {
...
void rx1619_set_pmi_icl(struct rx1619_chg *chip, int mA)
{
	...
	if (chip->wireless_by_usbin) {
		...
		if (chip->usb_psy)
			power_supply_set_property(chip->usb_psy, POWER_SUPPLY_PROP_CURRENT_MAX, &val);
...
void rx1619_get_pmi_icl(struct rx1619_chg *chip)
{
	...
	if (chip->wireless_by_usbin) {
		...
		if (chip->usb_psy)
			power_supply_get_property(chip->usb_psy, POWER_SUPPLY_PROP_CURRENT_MAX, &val);
	} else {
		if (!chip->dc_psy)
			chip->dc_psy = power_supply_get_by_name("dc");
...
*/
		...
		/* for usb-in design, set max usb icl to 1.8A*/
		for (i = 0; i <= 4; i++) {
			uA = (USB_20W_PLUS_BASE_CURRENT_UA + 200000 * i);
			rx1619_set_pmi_icl(chip, uA);
			msleep(100);
		}
		chip->target_curr = uA;
...
static void rx_set_charging_param(struct rx1619_chg *chip)
{
	...
	if (g_USB_TYPE >= 9) {
		...
		if (chip->is_pan_tx) {
			schedule_delayed_work(&chip->pan_tx_work, msecs_to_jiffies(0));
			goto out;
		}
		...
		case FULL_MODE:
			...
			if (batt_sts == POWER_SUPPLY_STATUS_CHARGING) {
				dev_info(chip->dev, "full mode -> recharge mode\n");
		...
		case RECHG_MODE:
			if (batt_sts == POWER_SUPPLY_STATUS_FULL) {
				dev_info(chip->dev, "recharge mode -> full mode\n");
				chip->status = FULL_MODE;
				chip->target_curr = SCREEN_OFF_FUL_CURRENT;
				chip->target_vol = ADAPTER_EPP_QC3_VOL;
				if (chip->wireless_psy) {
...
out:
	dev_info(chip->dev, "status:0x%x,adapter_vol=%d,icl_curr=%d,last_vin=%d,last_icl=%d\n",
			chip->status, chip->target_vol, chip->target_curr, last_vin, last_icl);
}
...
static void rx1619_fw_download_work(struct work_struct *work)
{
	...
	if (!val.intval) {
		...
		dev_info(chip->dev, "[rx1619] %s: FW Version is 0x%x\n", __func__, g_fw_rx_id);
		if (g_fw_rx_id)
			chip->chip_ok = 1;
		else
			chip->chip_ok = 0;
		...
		if ( is_valid_fw && (g_fw_rx_id >= FW_VERSION)) {
			...
			if (!rx1619_onekey_download_firmware(chip))
				...
				dev_info(chip->dev, "[rx1619] [%s] FW Update Success!\n", __func__);
				...
				dev_info(chip->dev, "boot_fw_version=0x%x, tx_fw_version=0x%x, rx_fw_version=0x%x\n",
		...
		dev_err(chip->dev, "%s: Skip FW download due to wireless charging\n", __func__);
}
...
static void rx1619_pan_tx_work(struct work_struct *work)
{
	...
	case FULL_MODE:
		...
		if (batt_sts == POWER_SUPPLY_STATUS_CHARGING) {
			dev_info (chip->dev, "[pan]full mode -> recharge mode\n");
			chip->status = RECHG_MODE;
			chip->target_curr = DC_LOW_CURRENT;
		}
		break;
	case RECHG_MODE:
		if (batt_sts == POWER_SUPPLY_STATUS_FULL) {
			dev_info (chip->dev, "[pan]recharge mode -> full mode\n");
			chip->status = FULL_MODE;
			chip->target_curr = SCREEN_OFF_FUL_CURRENT;
			if (chip->wireless_psy) {
				wk_val.intval = 0;
...
static void rx_chg_detect_work(struct work_struct *work)
{
	...
	if (val.intval || pc_val.intval) {
		dev_info(chip->dev, "usb_online:%d, pc online:%d set chip disable\n",
...
static void rx1619_enable_aicl( struct rx1619_chg * chip, bool enable){
	...
	if(chip->wireless_psy){
		if(chip->enabled_aicl != enable){
			...
			power_supply_set_property(chip->wireless_psy, POWER_SUPPLY_PROP_AICL_ENABLE, &val);
...
static void rx1619_wireless_int_work(struct work_struct *work)
{
	...
	if (tx_gpio) {
		if (rx1619_is_tx_mode(chip)) {
			...
			if (tx_status && tx_status != REVERSE_CHG_TX_SWITCH_DONE) {
				ret = rx_set_reverse_chg_mode(chip, false);
				chip->is_reverse_chg = 2;
				schedule_delayed_work(&chip->reverse_sent_state_work, 0);
			} else if (tx_status == REVERSE_CHG_TX_SWITCH_DONE) {
				dev_err(chip->dev, "[rx1619] restart because of OTG \n");
			}
...
*/
	...
	case 0x0f: //error cmd
		...
		if (!chip->epp && ((err_cmd == ID_CMD) ||
			...
			if (err_cmd == ID_CMD && (retry_id < 3)) {
				//send device idauth
				retry_id++;
				rx1619_retry_id_auth(chip);
				dev_info(chip->dev, "[rx1619] id auth failed retry %d\n", retry_id);
				break;
			} else if (err_cmd == AUTH_CMD && retry < 3){
				//send device auth
				rx1619_send_device_auth(chip);
				retry++;
				dev_info(chip->dev, "[rx1619] dev auth failed retry %d\n", retry);
				break;
			} else {
				retry = 0;
				retry_id = 0;
			...
			if (chip->epp){
				chip->target_vol = ADAPTER_EPP_QC3_VOL;
				if (chip->epp_max_power < 10000) {
					uA = ((chip->epp_max_power - 1000) / 11) * 1000;
					if (chip->op_mode == LN8282_OPMODE_SWITCHING)
						uA = uA * 2;
				} else {
					if (chip->op_mode == LN8282_OPMODE_SWITCHING)
						uA = USB_20W_PLUS_CURRENT_UA;
					else
						uA = USB_20W_PLUS_CURRENT_UA / 2;
				}
				rx1619_set_pmi_icl(chip, uA);
				rx1619_set_vout(chip, chip->target_vol);
			} else {
				for (i = 0; i < 7; i++) {
					uA = (100000 + 100000*i);
					rx1619_set_pmi_icl(chip, uA);
	...
	case 0x1f: //for product test
		...
		if (tx_req == 0x12) {//iout
			...
			dev_info(chip->dev, "[rx1619] [%s] product test--0x12--0x%x,0x%x iout=%d \n",
			...
			rx1619_write(chip, data_h, 0x0003);
			rx1619_write(chip, data_l, 0x0004);
			dev_info(chip->dev, "[rx1619] [%s] product test--0x13--0x%x,0x%x vout=%d \n",
			...
			dev_info(chip->dev, "[rx1619] [%s] product test--0x24--rxtx_fw_version=0x%x,0x%x\n",
			...
			dev_info(chip->dev, "[rx1619] [%s] product test--0x23--rx_hw_id=0x%x,0x%x\n",
									__func__, g_hw_id_h, g_hw_id_l);
			...
			dev_info(chip->dev, "[rx1619] [%s] product test--0x25--g_USB_TYPE=0x%x \n",
										__func__, g_USB_TYPE);
		} else
			dev_err(chip->dev, "[rx1619] [%s] product test--other cmd \n", __func__);
...
static int rx1619_set_reverse_gpio_state( struct rx1619_chg *chip, int enable){
	...
	if (chip->wireless_psy) {
		...
		power_supply_set_property(chip->wireless_psy, POWER_SUPPLY_PROP_REVERSE_GPIO_STATE, &reverse_val);
	} else {
		dev_err(chip->dev, "no wireless_psy,return\n");
		return -EINVAL;
	}
...
static int rx_set_reverse_chg_mode(struct rx1619_chg *chip, int enable)
{
	...
	if (gpio_is_valid(chip->tx_on_gpio)) {
		...
		if (enable) {
			...
			msleep(100);
			/* set reverse mode according to hw country */
			if (chip->hw_country == CountryGlobal || chip->is_otg_insert) {
				rx1619_set_reverse_parameter(chip, REVERSE_COUNTRY_INTERNATIONAL, REVERSE_CURRENT_500MA);
			} else {
				rx1619_set_reverse_parameter(chip, REVERSE_COUNTRY_DOMESTIC, REVERSE_CURRENT_1000MA );
			}
			/* set reverse fod */
		...
		char *buf)
{
	...
	if (g_chip->fw_update){
		dev_err(g_chip->dev, "[rx1619] [%s] Firmware Update is on going!\n",
						__func__);
	...
	if (!ret) {
		...
		tx_fw_version = (~fw_data_tx[sizeof(fw_data_tx)-1])&0xff;
		rx_fw_version = (~fw_data_rx[sizeof(fw_data_rx)-1])&0xff;
		boot_project_id_h = (~fw_data_boot[sizeof(fw_data_boot)-3])&0xff;
		boot_project_id_l = (~fw_data_boot[sizeof(fw_data_boot)-2])&0xff;
		...
		dev_info(g_chip->dev, "boot_fw_version=%c%c_V%02x, tx_fw_version=%c%c_V%02x, rx_fw_version=%c%c_V%02x \n",
		...
		char *buf)
{
	if (!g_chip->fw_update ) {
		...
		rx_set_reverse_gpio(g_chip, false);
		g_chip->fw_update = false;
		return scnprintf(buf, PAGE_SIZE, "app_ver:%02x.%02x.%02x.%x%x\n",
				g_fw_boot_id, g_fw_tx_id, g_fw_rx_id, g_hw_id_h,g_hw_id_l);
	} else {
		return scnprintf(buf, PAGE_SIZE, "fw update on going, cannot show version\n");
	}
}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (!ret) {
		dev_err(g_chip->dev, "[rx1619] [%s] RX Firmware Push failed! Please try again!\n",
						__func__);
		return scnprintf(buf, PAGE_SIZE, "RX Firmware Push failed! Please try again! \n");

	} else {
		dev_info(g_chip->dev, "rx_fw_version=%c%c_V%02x \n", rx_bin_project_id_h, rx_bin_project_id_l, rx_fw_version);
		dev_info(g_chip->dev, "[rx1619] [%s] RX Firmware Push Success!!! \n", __func__);
		return scnprintf(buf, PAGE_SIZE, "RX Firmware Push Success!!! rx_fw_ver=%c%c_V%02x\n",
				rx_bin_project_id_h, rx_bin_project_id_l, rx_fw_version);
	}
}
...
static DEVICE_ATTR(chip_fw_bin, S_IWUSR | S_IRUGO, attr_firmware_bin_show, attr_firmware_bin_store);
...
static int get_cmdline(struct rx1619_chg *chip)
{
	if (strnstr(saved_command_line, "androidboot.mode=",
		...
		dev_info(chip->dev, "[idtp9220]: enter power off charging app\n");
	} else {
...
*/
	...
	if (!g_rx1619_first_flag)
		schedule_delayed_work(&chip->rx_first_boot, msecs_to_jiffies(45000));
	return 0;
